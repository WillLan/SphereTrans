...
# input: (B, 128, 1024) output: (B, 128, 1024)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim =512,  dropout = 0.):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.layers = nn.ModuleList([])
        #print(dim, mlp_dim)
...
class SphereViT(nn.Module):
    def __init__(self, patch_size, dim, depth, heads, channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        self.pos_embedding = nn.Linear(2, dim//4)

        self.dropout = nn.Dropout(emb_dropout)

        self.transformer = Transformer(dim, depth, heads, dim_head, mlp_dim= 512, dropout=0)
    ...
    def forward(self, img, sph_pos_dis):
        ...
        #x = img.view(-1, 256, 512, 32).permute(0, 3, 1, 2).contiguous() # (B, H, W, C) --> (B, C, H, W)
