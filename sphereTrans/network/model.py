...
class SphereTransBlock(nn.Module):
    def __init__(self, depth_atten, dim, input_resolution, num_heads, dim_patch,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, token_projection='linear', token_mlp='leff',
                 se_layer=False, ref_point = None, sph_pos = None, flag = 0):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        ...
        self.dattn = SphereSelfAttention(depth_atten, num_heads, dim_patch, dim, k=9, last_feat_height=self.input_resolution[0], last_feat_width=self.input_resolution[1], dropout=0)
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, C)
        # W-MSA/SW-MSA
        x = self.dattn(x, self.ref_point.repeat(B, 1, 1, 1, 1), self.sph_pos)  # nW*B, win_size*win_size, C
...
class BasicTransformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth_atten, depth, num_heads, dim_patch,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.1, norm_layer=nn.LayerNorm, use_checkpoint=False,
                 token_projection='linear', token_mlp='leff', se_layer=False, ref_point = None, sph_pos = None, flag = 0):
        ...
        self.blocks = nn.ModuleList([
            SphereTransBlock(depth_atten = depth_atten, dim=dim, input_resolution=input_resolution,
                                  num_heads=num_heads, dim_patch = dim_patch,
                                  mlp_ratio=mlp_ratio,
                                  qkv_bias=qkv_bias, qk_scale=qk_scale,
                                  drop=drop, attn_drop=attn_drop,
                                  drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                                  norm_layer=norm_layer, token_projection=token_projection, token_mlp=token_mlp,
                                  se_layer=se_layer, ref_point=ref_point, sph_pos = sph_pos)
...
class SphereTrans(nn.Module):
    def __init__(self, img_size=256, in_chans=3,
                 embed_dim=32, depths_atten=[1, 1, 1, 1, 1, 1, 1, 1, 1], depths=[1, 1, 2, 2, 2, 2, 2, 1, 1],
                 num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2], dim_patch = [32, 16, 8, 4, 2, 4, 8, 16, 32],
        ...
        self.mlp = token_mlp
        self.ref_point256x512 = genSpherePoint(256, 512, 3, 3).cuda()#torch.load("network6/Equioffset256x512.pth")
        self.ref_point128x256 = genSpherePoint(128, 256, 3, 3).cuda()#torch.load("network6/Equioffset128x256.pth")
        self.ref_point64x128 = genSpherePoint(64, 128, 3, 3).cuda()#torch.load("network6/Equioffset64x128.pth")
        self.ref_point32x64 = genSpherePoint(32, 64, 3, 3).cuda()##torch.load("network6/Equioffset32x64.pth")
        self.ref_point16x32 = genSpherePoint(16, 32, 3, 3).cuda()#torch.load("network6/Equioffset16x32.pth")
        ...
        self.encoderlayer_0 = BasicTransformerLayer(dim=embed_dim,
                                                ...
                                                se_layer=se_layer, ref_point=self.ref_point256x512, sph_pos=self.sphere_postion256x512, flag = 0)
        ...
        self.encoderlayer_1 = BasicTransformerLayer(dim=embed_dim * 2,
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer, ref_point=self.ref_point128x256, sph_pos=self.sphere_postion128x256,  flag = 0)
        ...
        self.encoderlayer_2 = BasicTransformerLayer(dim=embed_dim * 4,
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer, ref_point=self.ref_point64x128,sph_pos=self.sphere_postion64x128, flag = 0)
        ...
        self.encoderlayer_3 = BasicTransformerLayer(dim=embed_dim * 8,
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,ref_point=self.ref_point32x64, sph_pos=self.sphere_postion32x64, flag = 0)
        ...
        self.conv = BasicTransformerLayer(dim=embed_dim * 16,
                                    ...
                                    depth_atten=depths_atten[4],
                                      ...
                                      norm_layer=norm_layer,
                                      use_checkpoint=use_checkpoint,
                                      token_projection=token_projection, token_mlp=token_mlp, se_layer=se_layer,ref_point=self.ref_point16x32, sph_pos=self.sphere_postion16x32, flag = 0)
        ...
        self.decoderlayer_0 = BasicTransformerLayer(dim=embed_dim * 16,
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,ref_point=self.ref_point32x64, sph_pos=self.sphere_postion32x64, flag = 1)
        ...
        self.decoderlayer_1 = BasicTransformerLayer(dim=embed_dim * 8,
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,ref_point=self.ref_point64x128, sph_pos=self.sphere_postion64x128, flag = 1)
        ...
        self.decoderlayer_2 = BasicTransformerLayer(dim=embed_dim * 4,
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer,ref_point=self.ref_point128x256, sph_pos=self.sphere_postion128x256, flag = 1)
        ...
        self.decoderlayer_3 = BasicTransformerLayer(dim=embed_dim * 2,
                                                ...
                                                use_checkpoint=use_checkpoint,
                                                token_projection=token_projection, token_mlp=token_mlp,
                                                se_layer=se_layer, ref_point=self.ref_point256x512, sph_pos=self.sphere_postion256x512, flag = 1)
    ...
    def forward(self, x):
        ...
        up1 = self.upsample_1(deconv0)  # (64, 128)
