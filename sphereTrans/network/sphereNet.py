...
def cal_index(h, w, img_r, img_c):
    ...
    '''
    # pixel -> rad
    phi = -((img_r+0.5)/h*pi - pi/2)    # 中心点坐标
    theta = (img_c+0.5)/w*2*pi-pi

    delta_phi = pi/h    # 角度分辨率
    delta_theta = 2*pi/w
...
def gen_grid_coordinates(h, w, stride=1):
    ...
    coordinates = coordinates[::-1] # 调换x,y;现在的x对应图像的H,y对应图像的W，而下面的sample函数传入的grid中xy是按照坐标系的坐标采样的，所以要调换过来
    # 这里[::-1]是对第0维的数组进行调换，这里是coordinates[0, ...]和[1, ...]进行调换，而数组内部的元素不进行调换
    coordinates = coordinates.transpose(1, 3, 2, 4, 0)
    sz = coordinates.shape
    coordinates = coordinates.reshape(1, sz[0]*sz[1], sz[2]*sz[3], sz[4]) # (1, H*3, W*3, 1)
...
if __name__ == '__main__':
    ...
    model = SphereConv2D(in_c=3, out_c=256, stride=2)
    pic = torch.randn(1,3,256,512)
    out = model(pic)
    print(out.shape)
    # test cnn
    # cnn = SphereConv2D(3, 3, 1)
    # cnn1 = SphereConv2D(3, 3, 1)
    # out = cnn(torch.randn(2, 3, 10, 10))
    # print('SphereConv2D(3, 5, 1) output shape: ', out.size())
    # # test pool
    # # create sample image
    #
    # rgb_name = os.path.join('/media/yl/s2d3d/area_3/pano/rgb/camera_0ccf3c78ef354902b516c62ef8fb7cf1_lounge_2_frame_equirectangular_domain_rgb.png')
    # # print(rgb_name)
    ...
    # img = np.expand_dims(rgb, 0)
    # img = torch.Tensor(img)
    # out1 = cnn1(cnn(img))
    # out1 = np.squeeze(out1.detach().numpy(), 0).transpose([1, 2, 0])
    # plt.imsave('demo_pool_3.png', (out1-out1.min())/(out1.max() - out1.min()))
    ...
    # from network.model import SphereTrans as SphereTrans
    # img = torch.randn(2, 3, 512, 1024).cuda()
    # net = SphereTrans()
    # img_out = net(img)
    # print(img_out.shape)
