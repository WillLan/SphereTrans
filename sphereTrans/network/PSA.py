...
from models.vit import AttentionBlock, SphereViT
...
class SphereSelfAttention(nn.Module):
    def __init__(self, depth_atten, h, dim_patch,
                 d_model,
                 k,
                 last_feat_height,
                 last_feat_width,
        ...
        self.sphereVit = SphereViT(patch_size=dim_patch, dim=1024, depth=2, heads=8, channels = d_model, dim_head = 64)
        self.conv = nn.Conv2d(d_model, d_model, kernel_size=3, stride=1, padding=1)



    def forward(self, input, sphere_point, sph_pos):
        ...
        # 按照球面坐标从ERP图像中sample值，然后与球面坐标的embedding相加
        ...
        #print(input.shape)
        sph_pt_feat_emd = self.point_proj(sph_pt_feat) # (B, 3H, 3W, C)
        ...
        # 全局的self-attention操作
        output_global = self.sphereVit(output_att, sph_pos) #(B, C, H, W)
        output_global = self.conv(output_global)

        return output_global.permute(0, 2, 3, 1).contiguous()  # (B, H, W, C)
